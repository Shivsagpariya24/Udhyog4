...
<?php
    ...
    if (isset($_REQUEST['nm'])) {
        ...
        $qua    = stripslashes($_REQUEST['qua']);
        $qua    = mysqli_real_escape_string($con, $qua);
        $exp    = stripslashes($_REQUEST['exp']);
        $exp    = mysqli_real_escape_string($con, $exp);
        $f_test = stripslashes($_REQUEST['f_test']);
        $f_test = mysqli_real_escape_string($con, $f_test);
        $req    = stripslashes($_REQUEST['req']);
        $req    = mysqli_real_escape_string($con, $req);
        $remark = stripslashes($_REQUEST['remark']);
        $remark = mysqli_real_escape_string($con, $remark);
        
        $query    = "INSERT into `qr3` (nm,des,qua,exp,f_test,req,remark)
                    VALUES ('$nm','$des','$qua','$exp','$f_test','$req','$remark')";
        $result   = mysqli_query($con, $query);
        if ($result) {
            ?>
                <script>
                    alert ('your data will be successfully submit');
                    location.replace('qr-3.php');
                </script>
                ...
                <div class="col-11">
                    ...
                    <div class="row mt-2" style="font-size: 23px;">
                        <div class="col-12 d-flex justify-content-between align-items-center"
                            ...
                            <a href="qr-3.php" class="abc ml-5">New</a>
                            <a href="modify/qr3.php" class="abc ml-5">Modify</a>
                            <a href="report/qr3.php" class="abc ml-5">Report</a>
                            </div>
                            </div>
                            <div class="">
