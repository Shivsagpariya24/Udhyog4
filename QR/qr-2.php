...
<?php
    ...
    if (isset($_REQUEST['doc_des'])) {
        // removes backslashes
        $doc_des = stripslashes($_REQUEST['doc_des']);
        $doc_des = mysqli_real_escape_string($con, $doc_des);
        $dep    = stripslashes($_REQUEST['dep']);
        $dep    = mysqli_real_escape_string($con, $dep);

        $upl = $_FILES['upl']['name'];
        $tmp_name = $_FILES['upl']['tmp_name'];
        $final_file = "File/qr2/".$upl;
        $upload = move_uploaded_file($tmp_name,$final_file);

        $query    = "INSERT into `qr2` (doc_des,dep,upl)
                    VALUES ('$doc_des','$dep','$upl')";
        $result   = mysqli_query($con, $query);
        if ($result) {
            ?>
                <script>
                    alert ('your data will be successfully submit');
                    location.replace('qr-2.php');
                </script>
	...
	<div class="container-fluid">
                ...
                <div class="col-11">
                    ...
                    <div class="row mt-2" style="font-size: 23px;">
                        <div class="col-12 d-flex justify-content-between align-items-center"
                            ...
                            <a href="qr-2.php" class="abc ml-5">New</a>
                            <a href="modify/qr2.php" class="abc ml-5">Modify</a>
                            <a href="report/qr2.php" class="abc ml-5">Report</a>
                            </div>
                            </div>
                            <div class="">
        ...
        <form action="" method="post" enctype="multipart/form-data">
           ...
           <div class="col-4">
                    ...
                    <input type="text" name="doc_des" class="form-control">
                </div>
                <div class="col-4">
