...
<?php
    ...
    if (isset($_REQUEST['title'])) {
        ...
        $d_no    = stripslashes($_REQUEST['d_no']);
        $d_no    = mysqli_real_escape_string($con, $d_no);
        $r_no    = stripslashes($_REQUEST['r_no']);
        $r_no    = mysqli_real_escape_string($con, $r_no);
        $i_no    = stripslashes($_REQUEST['i_no']);
        $i_no    = mysqli_real_escape_string($con, $i_no);
        $i_date    = stripslashes($_REQUEST['i_date']);
        $i_date    = mysqli_real_escape_string($con, $i_date);
        $re_no    = stripslashes($_REQUEST['re_no']);
        $re_no    = mysqli_real_escape_string($con, $re_no);
        $p_no    = stripslashes($_REQUEST['p_no']);
        $p_no    = mysqli_real_escape_string($con, $p_no);
        $np    = stripslashes($_REQUEST['np']);
        $np    = mysqli_real_escape_string($con, $np);
        $upl = $_FILES['upl']['name'];
        $tmp_name = $_FILES['upl']['tmp_name'];
        $final_file = "upload/tp/".$upl;
        $upload = move_uploaded_file($tmp_name,$final_file);
        

        $query    = "INSERT into `lot` (title,d_no,r_no,i_no,i_date,re_no,p_no,np,upl)
                    VALUES ('$title','$d_no','$r_no','$i_no','$i_date','$re_no','$p_no','$np','$upl')";
        $result   = mysqli_query($con, $query);
        if ($result) {
            ?>
                <script>
                    alert ('your data will be successfully submit');
                    location.replace('list_of_tp.php');
                </script>
                ...
                <div class="col-11">
                    ...
                    <div class="row mt-2" style="font-size: 23px;">
                        <div class="col-12 d-flex justify-content-between align-items-center"
                            ...
                            <a href="list_of_tp.php" class="abc ml-5">New</a>
                            <a href="Modify/LOT.php" class="abc ml-5">Modify</a>
                            <a href="Report/LOT.php" class="abc ml-5">Report</a>
                            </div>
                            </div>
                            <div class="">
        ...
        <br>
            <div class="row">
                ...
                <div class="col-4">
                    ...
                    <input type="date" name="i_date" class="form-control">
                </div>                
                 <div class="col-4">
