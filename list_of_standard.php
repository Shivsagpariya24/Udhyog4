...
<?php
    ...
    if (isset($_REQUEST['no'])) {
        ...
        $old_edition    = stripslashes($_REQUEST['old_edition']);
        $old_edition    = mysqli_real_escape_string($con, $old_edition);
        $l_version    = stripslashes($_REQUEST['l_version']);
        $l_version    = mysqli_real_escape_string($con, $l_version);

        $upl = $_FILES['upl']['name'];
        $tmp_name = $_FILES['upl']['tmp_name'];
        $final_file = "upload/st/".$upl;
        $upload = move_uploaded_file($tmp_name,$final_file);

        $query    = "INSERT into `los` (no,year,des,old_edition,l_version,upl)
                    VALUES ('$no','$year','$des','$old_edition','$l_version','$upl')";
        $result   = mysqli_query($con, $query);
        if ($result) {
            ?>
                <script>
                    alert ('your data will be successfully submit');
                    location.replace('list_of_standard.php');
                </script>
...
<!-- Navbar -->
                ...
                <div class="col-11">
                    ...
                    <div class="row mt-2" style="font-size: 23px;">
                        <div class="col-12 d-flex justify-content-between align-items-center"
                            ...
                            <a href="list_of_standard.php" class="abc ml-5">New</a>
                            <a href="Modify/LOS.php" class="abc ml-5">Modify</a>
                            <a href="Report/LOS.php" class="abc ml-5">Report</a>
                            </div>
                            </div>
                            <div class="">
    ...
    <div class="float-right" style="width: 91%; ">
        ...
        <br>
            <div class="row">
                <div class="col-4">
                    <label >Old Edition</label>
                    <input type="text" name="old_edition" class="form-control">
                </div>                
                 <div class="col-4">
                    <label >Letest Version</label>
                    <input type="text" name="l_version" class="form-control">
                </div>
                <div class="col-4">
                    <label>Upload</label>
